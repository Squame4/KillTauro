...
public class Beshtia implements Mostro,Runnable
{
	...
	public Beshtia(int x_ , int y_ , PannelloDiGioco p)
	{
		x = x_;
		y = y_;
		pannello = p;
		...
		width = 150; //dim sprite
		height = 64;
		
		hitBox = new Rectangle(x, y, 88, 32);
	...
	private void caricaSprite() {
		...
		for (int i = 0; i < 6; i++) {
			try {
				sprite.add(ImageIO.read(new File("Wolf" + File.separator + i + ".png")));
			} catch (IOException e) {
...
//		g2d.drawLine(x+20 , y-3 , x+80 , y-3);
		...
		if (dx > 0)
			...
			g2d.drawImage(imgCorrente, x + 100, y - 31, -width, height, null);
	...
	public void playSuonoMorte() {
		urlMorteLupo = Menu.class.getResource("ost/LUPO.wav");
		morteLupo = Applet.newAudioClip(urlMorteLupo);
		morteLupo.play();
