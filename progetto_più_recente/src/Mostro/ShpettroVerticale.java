...
public class ShpettroVerticale implements Mostro,Runnable
{
	...
	public ShpettroVerticale(int x_ , int y_ , PannelloDiGioco p)
	{
		x = x_;
		y = y_;
		pannello = p;
		...
		dx = 0; //il movimento iniziale è a destra
		dy = 3;
	...
	public void muovi(int visualeX) {
		...
		for (Blocco i : pannello.getLivelli().get(pannello.getIndexLivelloCorrente()).getBlocchi()) 
		{
					if (hitBox.intersects(i.getHitBox()) && (i.getTipo() >= 1 || i.getTipo() == -3)) 
					{
						hitBox.y -= dy;
						dy = -dy;
						hitBox.y = y; 
